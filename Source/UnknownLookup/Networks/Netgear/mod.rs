...
use crate::UnknownLookup::{regex_and_default_to_empty_string, SearchType::{DeviceAttributeSearch, NetworkSearch}};


//TODO: Rework to get address, label, mac, etc.
pub fn convert_section_to_device(device: &DeviceAttributeSearch, network: &NetworkSearch, section: &String) -> Device
{
	...
	return Device{address: format!("{:?}", device), label: label, is_reservation: false, is_static: false, mac: Some(mac),
	  groups: vec![], Network: network.network()};
}


pub fn device_expression(device: &DeviceAttributeSearch) -> String
{
