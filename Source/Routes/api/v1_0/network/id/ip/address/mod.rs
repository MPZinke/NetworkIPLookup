...
use crate::Query::Queries::{Network::SELECT_Network_by_id, IP::SELECT_IP_by_Network_id_AND_IP_address};
...
pub async fn index() -> HttpResponse
{
	let body: &str = r#"
	{
		"/api/v1.0/network/id/{network_id}/ip/address/{ip_address}": "Get an IP by IP address and network id"
	}
	"#;
...
pub async fn address(auth: BearerAuth, path: web::Path<(i32, String)>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<IP, Error> = SELECT_IP_by_Network_id_AND_IP_address(pool.as_ref(), Network_id,
	  &IP_address).await;
