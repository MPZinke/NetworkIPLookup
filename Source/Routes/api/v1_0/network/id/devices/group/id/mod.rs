...
use crate::Query::{query_to_response, Queries::Device::SELECT_Devices_by_Network_id_AND_Group_id};
...
pub async fn id(auth: BearerAuth, path: web::Path<(i32, i32)>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<Vec<Device>, Error> = SELECT_Devices_by_Network_id_AND_Group_id(pool.as_ref(), Network_id,
	  Group_id).await;
