...
use crate::Query::{Network::SELECT_Network_by_id, Device::SELECT_Device_by_Network_id_AND_Device_address};
...
pub async fn address(auth: BearerAuth, path: web::Path<(i32, String)>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let device_result: Result<Device, LookupError> = SELECT_Device_by_Network_id_AND_Device_address(pool.as_ref(),
	...
	if(query_NotFound(&device_result))
	{
		...
		if(network_search.network().auth_value.is_some())
		{
			...
			let lookup_result: Result<Device, LookupError> = lookup_device(&address_attribute, &network_search).await;
