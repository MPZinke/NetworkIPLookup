...
use crate::Query::{query_to_response, Queries::Device::SELECT_Devices_by_Network_label};
...
pub async fn all(auth: BearerAuth, path: web::Path<String>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<Vec<Device>, Error> = SELECT_Devices_by_Network_label(pool.as_ref(), &label).await;
