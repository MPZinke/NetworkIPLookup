...
use crate::Query::{query_to_response, Queries::Device::SELECT_Devices_by_Network_label_AND_Group_id};
...
pub async fn id(auth: BearerAuth, path: web::Path<(String, i32)>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<Vec<Device>, Error> = SELECT_Devices_by_Network_label_AND_Group_id(pool.as_ref(), &Network_label,
	  Group_id).await;
