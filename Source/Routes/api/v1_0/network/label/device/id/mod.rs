...
use crate::Query::{query_to_response, Queries::Device::SELECT_Device_by_Network_label_AND_Device_id};
...
pub async fn id(auth: BearerAuth, path: web::Path<(String, i32)>, pool: web::Data<PgPool>) -> HttpResponse
{
	...
	let query_response: Result<Device, Error> = SELECT_Device_by_Network_label_AND_Device_id(pool.as_ref(), &Network_label,
	  Device_id).await;
